...
fn process_transaction(
    ...
    let wallet_encoded = {
        ...
        match read_trie.get(&key) {
            ...
            _ => {
                println!("Target wallet not found");
                return;
            }
        }
    };
    ...
    let target_wallet_encoded = {
        let read_trie = trie.read().unwrap();
        match read_trie.get(&target_wallet_id.to_be_bytes()) {
            Ok(Some(wallet)) => wallet.clone(),
            _ => {
                println!("Target wallet not found");
                return;
...
fn process_roots(
    ...
    if seen_roots.insert(root.clone()) {
        ...
        let shared_trie = trie.clone();
        std::thread::spawn(move || {
            let start = std::time::Instant::now();
            let mut write_trie = shared_trie.write().unwrap();
            while let Ok((key, encoded)) = updates_consumer.recv_timeout(Duration::from_millis(1)) {
